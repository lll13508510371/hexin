...
"""
"""自己在下方编写代码实现功能"""
# print(data['data']['artist'][0]['artistTypeName']+': '+data['data']['artist'][0]['name'])
# print('歌名' + ': ' + data['data']['title'])
# print('音乐地址' + ':' + data['data']['path'])
'''
!!! ''""就近原则,下面我刚开始写用的全是''
'''
print(f'歌手:{data["data"]["artist"][0]["name"]}\n',
      f"歌名:{data['data']['title']}\n",
      f"音乐地址:{data['data']['path']}",
      sep='')
'''
!!! 这里不设置sep的话print默认的sep是' ',看到的结果就会有一个空白字符(一个空格)
'''
# print(data['data']['artist'][0]['name'],data['data']['title'],data['data']['path'])
