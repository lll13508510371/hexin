...
class NetGui:
    def __init__(self, WINDOW_SIZE='650x500+50+50'):
        ...
        w = ttk.Combobox(network_frame,
                         textvariable=self.agreement_type_variable,
                         values=("TCP 服务器", "TCP 客户端"))
        w.pack(fill=tk.X)

        tk.Label(network_frame, text='(2) 本地主机地址').pack(anchor=tk.W)
        host = ttk.Combobox(network_frame, textvariable=self.host_variable,
                            values=hosts_list)
        host.pack(fill=tk.X)
        ...
        """接收数据"""
        recv_setting_frame = tk.LabelFrame(left_frame, text="接收设置", pady=5,
                                           padx=5)
        recv_setting_frame.pack(side=tk.TOP, anchor=tk.N, fill=tk.X)

        tk.Radiobutton(recv_setting_frame, text="utf-8",
                       variable=self.recv_hex_var, value=1).pack(anchor=tk.W)
        tk.Radiobutton(recv_setting_frame, text="gbk",
                       variable=self.recv_hex_var, value=2).pack(anchor=tk.W)

        tk.Checkbutton(recv_setting_frame, text='json数据',
                       variable=self.recv_v_int[0]).pack(anchor='w')
        tk.Checkbutton(recv_setting_frame, text='自动换行',
                       variable=self.recv_v_int[1]).pack(anchor='w')

        """发送数据"""
        send_setting_frame = tk.LabelFrame(left_frame, text="发送设置", pady=5,
                                           padx=5)
        send_setting_frame.pack(anchor=tk.N, fill=tk.X)

        tk.Radiobutton(send_setting_frame, text="utf-8",
                       variable=self.send_hex_var, value=1).pack(anchor=tk.W)
        tk.Radiobutton(send_setting_frame, text="gbk",
                       variable=self.send_hex_var, value=0).pack(anchor=tk.W)

        tk.Checkbutton(send_setting_frame, text='数据加密(未实现)',
                       variable=self.send_v_int[0]).pack(anchor='w')
        tk.Checkbutton(send_setting_frame, text='信息接受(未实现)',
                       variable=self.send_v_int[1]).pack(anchor='w')
...
class Net(NetGui):
    ...
    def create_server_socket(self, inet=socket.AF_INET,
    ...
    def open_client_socket(self, inet=socket.AF_INET, sock=socket.SOCK_STREAM):
        ...
        # 处理服务器的数据接收
        threading.Thread(target=self.handle_client_recv,
                         args=(self.client_socket,)).start()
        # 绑定发送信息事件
        self.send_button.config(command=self.send_message_thread)
